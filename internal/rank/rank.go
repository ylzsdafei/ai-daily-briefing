...
- 某些 item 后会附带热度信号, 如 "stars+=10051 trending#1 mentions=8". 三个维度:
  * stars+= 是本周 GitHub star 新增数 (越大越热)
  * trending# 是 ossinsight 排名 (越小越靠前)
  * mentions= 是该 repo 在非 ossinsight 源 (科技媒体/博客/论坛) 被提及次数
- 评分要综合三个维度 + repo description 做"业务价值"判断:
  * 同时满足 (a) 高 star (stars+>2000 或 trending#<=10) + (b) 有跨源讨论 (mentions>=3) + (c) description 有具体业务场景/产品特色 → **给 9-10 分**, 必选
  * 高 star 但无跨源讨论 (或反之) + description 平庸 → 7-8 分
  * trending 但 description 像 demo/toy, 无明显业务价值 → 5-6 分
- 目标: Hermes / VibeVoice 这种"GitHub 新星 + 科技媒体热议 + 有业务价值"的项目必须入选, 不要被描述花哨但实际冷门的 repo 挤掉
...
func formatRepoHotnessExtra(it *store.RawItem) string {
	...
	// 只对 ossinsight metadata 提取 stars/rank (其他 adapter 的 metadata
	// 格式不同, 静默跳过).
	var meta struct {
	...
	parts := []string{}
	if meta.Stars != "" && meta.Stars != "0" {
		parts = append(parts, fmt.Sprintf("stars+=%s", meta.Stars))
	}
	if meta.Rank > 0 {
