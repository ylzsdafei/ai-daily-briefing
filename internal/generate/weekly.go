...

{
  ...
  "focus": "本周聚焦 — 选 2-3 件本周最重要的事做深度拆解。每件事: 发生了什么 → 为什么重要 → 对行业意味着什么。共 1200-1800 字。使用 markdown 格式，每件事结构如下: 先 ### 小标题，紧接着一段 mermaid 图（三个反引号mermaid围栏），然后才是文字分析。读者先看图抓住核心逻辑，再读文字了解细节。图的要求: graph LR 格式(从左到右); 只要 4-6 个节点; 一条主线(A-->|转折词|B-->|转折词|C-->|转折词|D); 边标签用 -->|标签| 语法; 节点文字极简(4-8字); classDef 用 blue/yellow/green(不要用 start/end 保留字，行末不加分号); 起点 blue，中间 yellow，终点 green; 不要 subgraph。",
