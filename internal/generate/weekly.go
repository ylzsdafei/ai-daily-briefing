...

{
  ...
  "trends_diagram": "一段 mermaid 代码（不含围栏标记），画一张简版'本周核心脉络图'。设计原则: 让不懂技术的人 5 秒看懂。要求: graph LR; 3-4 个趋势大节点串联; classDef blue fill:#dbeafe,stroke:#3b82f6 染色; 总共 5-7 个节点; 文字极简(4-10字); 边标签用 -->|词| 语法; 不用 subgraph; 不用 start/end 保留字; classDef 行末不加分号。",
  "trends_diagram_detail": "一段 mermaid 代码（不含围栏标记），画一张详版趋势全景图。要求: graph TD; 用 subgraph 按趋势方向分组(每组 2-3 个事件节点); 组之间用虚线(-.->|关系词|)连接; classDef blue fill:#dbeafe,stroke:#3b82f6 / classDef yellow fill:#fef3c7,stroke:#f59e0b / classDef green fill:#d1fae5,stroke:#10b981 / classDef purple fill:#ede9fe,stroke:#8b5cf6; 每个 subgraph 内节点用不同颜色; 12-16 个节点; 边标签用 -->|词| 语法; 不用 start/end 保留字; classDef 行末不加分号。",
