...

{
  ...
  "focus": "本周聚焦 — 选 2-3 件本周最重要的事做深度拆解。每件事: 发生了什么 → 为什么重要 → 对行业意味着什么。共 1200-1800 字。使用 markdown 格式，每件事用 ### 小标题分隔。每件事的分析末尾附一段 mermaid 图（三个反引号mermaid围栏）。图的要求: graph TD 格式; 用 subgraph 分组(事件层/影响层/启示层); 节点用不同形状(圆角矩形=事件,菱形=判断,六边形=趋势,圆形=结论); 用 classDef 定义颜色类(event fill:#dbeafe,stroke:#3b82f6 / impact fill:#fef3c7,stroke:#f59e0b / insight fill:#d1fae5,stroke:#10b981); 边上标关系词; 8-12 个节点。",
  ...
  "trends_diagram": "一段 mermaid 代码（不含围栏标记），用 graph TD 画一张'本周行业全景图'。要求: 每个趋势方向用 subgraph 作为一个色块区域(标题用趋势名称); 每个 subgraph 内放 2-3 个具体事件节点; subgraph 之间用虚线箭头(-.->)标注跨趋势的影响关系; 用 classDef 定义 4 种颜色类分别给 4 个区域的节点着色(蓝#dbeafe/#3b82f6 黄#fef3c7/#f59e0b 绿#d1fae5/#10b981 紫#ede9fe/#8b5cf6); 用 style 给每个 subgraph 设置浅色背景; 12-18 个节点。这张图的目标是让读者一眼看到'本周 AI 行业发生了什么、各个方向怎么关联'，像一张行业地图。",
