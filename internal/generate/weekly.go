...

{
  ...
  "focus": "本周聚焦 — 选 2-3 件本周最重要的事做深度拆解。每件事: 发生了什么 → 为什么重要 → 对行业意味着什么。共 1200-1800 字。使用 markdown 格式，每件事用 ### 小标题分隔。每件事的分析末尾附一段 mermaid 图（三个反引号mermaid围栏）。图的设计原则: 让读者3秒看懂这件事的核心逻辑。要求: graph LR 格式(从左到右，像时间线); 只要 4-6 个节点; 一条主线串联(A-->|转折词|B-->|转折词|C-->|转折词|D); 边标签必须用 -->|标签| 语法(不要用 -- 标签 --> 语法); 节点文字极简(4-8字); 用 classDef 定义三种颜色(名字用 blue/yellow/green，不要用 start/end 等保留字): classDef blue fill:#dbeafe,stroke:#3b82f6 / classDef yellow fill:#fef3c7,stroke:#f59e0b / classDef green fill:#d1fae5,stroke:#10b981。起点用 blue，中间用 yellow，终点用 green。classDef 行末尾不要加分号; 不要 subgraph，不要复杂分支，越简单越好。",
  ...
  "trends_diagram": "一段 mermaid 代码（不含围栏标记），画一张'本周核心脉络图'。设计原则: 让不懂技术的人 5 秒看懂本周 AI 行业在往哪走。要求: graph LR 格式; 用 3-4 个大节点代表本周趋势方向，用 2-3 个小节点代表关键事件; 大节点之间用箭头串联表示趋势走向; 事件节点用虚线(-.->)连到对应趋势; 大节点用 classDef 染蓝(fill:#dbeafe,stroke:#3b82f6,font-weight:bold), 事件节点染浅灰(fill:#f1f5f9,stroke:#94a3b8); 总共 6-9 个节点, 不超过 10 个; 文字极简(4-10字)。不要 subgraph, 保持扁平简洁。",
