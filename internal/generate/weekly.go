...

{
  ...
  "focus": "本周聚焦 — 选 2-3 件本周最重要的事做深度拆解。每件事: 发生了什么 → 为什么重要 → 对行业意味着什么。共 1200-1800 字。使用 markdown 格式，每件事用 ### 小标题分隔。每件事的分析末尾附一段 mermaid 图（三个反引号mermaid围栏）。图的设计原则: 让读者3秒看懂这件事的核心逻辑。要求: graph LR 格式(从左到右，像时间线); 只要 4-6 个节点; 一条主线串联(A-->|转折词|B-->|转折词|C-->|转折词|D); 边标签必须用 -->|标签| 语法(不要用 -- 标签 --> 语法); 节点文字极简(4-8字); 用 classDef 给起点染蓝(fill:#dbeafe,stroke:#3b82f6)、终点染绿(fill:#d1fae5,stroke:#10b981)、中间节点染黄(fill:#fef3c7,stroke:#f59e0b); 不要 subgraph，不要复杂分支，越简单越好。",
